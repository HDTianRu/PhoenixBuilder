...
var I18nDict_torrekie map[uint16]string = map[uint16]string{
	...
	Auth_FailedToRequestEntry:           "未能请求租赁服入口，给爷检查租赁服等级设置是否关闭及租赁服密码是否正确。",
	Auth_HelperNotCreated:               "辅助用户尚未创建，给爷前往用户中心进行创建。",
	Auth_InvalidFBVersion:               "FastBuilder 版本无效，给爷更新。",
	Auth_InvalidHelperUsername:          "辅助用户的用户名无效，给爷前往用户中心进行设置。",
	Auth_InvalidToken:                   "无效Token，给爷重新登录。",
	Auth_InvalidUser:                    "无效用户，给爷重新登录。",
	Auth_ServerNotFound:                 "租赁服未找到，给爷检查租赁服是否对所有人开放。",
	Auth_UnauthorizedRentalServerNumber: "对应租赁服号尚未授权，给爷前往用户中心进行授权。",
	Auth_UserCombined:                   "该用户已经合并到另一个账户中，给爷使用新账户登录。",
	Auth_FailedToRequestEntry_TryAgain:  "未能请求租赁服入口，给爷稍后再试。",
	BDump_Author:                        "作者",
	...
	ERRORStr:                            "错误",
	EnterPasswordForFBUC:                "给爷输入你的FastBuilder用户中心登录密码(不会显示): ",
	Enter_FBUC_Username:                 "输入你的FastBuilder用户中心用户名: ",
	Enter_Rental_Server_Code:            "给爷输入租赁服号: ",
	...
	Get_Warning:                         "get命令已被set命令取代，未来将移除此命令，给爷迁移至set",
	IgnoredStr:                          "已忽略",
	...
	Notice_CheckUpdate:                  "正在检查更新，给爷稍等…",
	...
	Notice_UpdateNotice:                 "给爷更新本软件。\n",
	Notice_ZLIB_CVE:                     "您的zlib版本（%s）包含已被证实的严重漏洞，我们建议您更新它，以免发生意外",
	Notify_NeedOp:                       "需要 OP 权限以正常工作。",
	Notify_TurnOnCmdFeedBack:            "需要 sendcommandfeedback 为 true，我们已经为你打开该选项，使用完后给爷按需关闭",
	Omega_WaitingForOP:                  "Omega系统正在等待 OP 权限",
	Omega_Enabled:                       "Omega系统已启用！",
	OpPrivilegeNotGrantedForOperation:   "未授予机器人 OP 权限，给爷授予 OP 权限后再进行此操作",
	...
	SelectLanguageOnConsole:             "给爷在控制台中选择新语言",
	...
	Special_Startup:                     "已启用语言：torrekie语\n",
	/* Special SysError Translations, for innocent kids */
	SysError_EACCES:             "权限拒绝，给爷检查是否已经允许该程序访问对应文件。",
	SysError_EBUSY:              "文件被占用，给爷稍后再试。",
	SysError_EINVAL:             "无效文件输入。",
	SysError_EISDIR:             "输入文件为目录，无效输入。",
	SysError_ENOENT:             "对应文件不存在。",
	SysError_ETXTBSY:            "文件被占用，给爷稍后再试。",
	...
	Warning_ACME_Deprecated:     "警告 - `acme' 功能已弃用，将在未来被移除。给爷迁移到其他文件格式。\n",
	Warning_UserHomeDir:         "警告 - 爷无法获取当前用户主目录，将设定homedir=\".\";\n",
}
